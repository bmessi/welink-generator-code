...
//        context.setTargetRuntime("MyBatis3Simple");
    ...
    public void genMuliCodeWithCustomName(Map<String, String> map) {
        for (Map.Entry<String, String> entry : map.entrySet()) {
            genCodeByTableName(entry.getKey(), entry.getValue(), false);
        }
    }
    ...
    private void genCodeByTableName(String tableName, String modelName, boolean flag) {
        ...
        new MapperGenerator().genCode(tableName, modelName);
...
//        pluginConfiguration.setConfigurationType("com.com.codegen.service.plugin.CustomPlugin");
    ...
    private static void init() {
        ...
        String dateFormat = "".equals(prop.getProperty("date-format")) ? "yyyy-MM-dd HH:mm:ss" : prop.getProperty("date-format");
        DATE = new SimpleDateFormat(dateFormat).format(new Date());

        IGNOREFIELDS = prop.getProperty("ignoreFields");
    }
    ...
    private static Properties loadProperties() {
        ...
        try {
            prop = new Properties();
            InputStream in = com.codegen.service.CodeGeneratorManager.class.getClassLoader().getResourceAsStream("generatorConfig.properties");
            prop.load(in);
        } catch (Exception e) {
